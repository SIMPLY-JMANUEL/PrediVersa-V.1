...
const DICCIONARIO = {
  ...
  violencia_fisica: {
    ...
    palabras: [
      ...
      'me dieron una tunda', 'me sonaron', 'me dieron sus lamparazos', 'me cascaron feo'
    ]
  },
  ...
  bullying_verbal: {
    ...
    palabras: [
      ...
      'me boletearon', 'me la tienen velada'
    ]
  },
  ...
  grooming_sexting: {
    ...
    palabras: [
      'mandame una foto', 'pasa el pack', 'video llamada solos', 'sin ropa', 
      'nuestro secreto', 'no le cuentes a tus papas', 'no le digas a nadie', 
      'te doy un regalo si', 'eres muy linda para tu edad', 'te ves muy bien',
      'mostrame la camara', 'pasa fotos intimas', 'sexting'
    ]
  }
}
...
function analyzeText(texto, tipoViolencia = '', frecuencia = '', historial = []) {
  ...
  if (numReportes > 0) {
    // 1. Bonus por reincidencia con DECAIMIENTO (más reciente = más impacto)
  ...
  // ── CAPA 5: UMBRALES DINÁMICOS PARA GEMINI ──────────
  // Si es acoso sexual o amenazas graves, bajamos el umbral para asegurar validación IA
  let thresholdGemini = 30;
  const requiereValidacionTemprana = tiposViolenciaDetectados.some(t => 
    ['acoso_sexual', 'grooming_sexting', 'amenazas'].includes(t)
  );
  if (requiereValidacionTemprana) thresholdGemini = 22;
  ...
  return {
    ...
    requiere_gemini: score >= thresholdGemini && score < 67, 
